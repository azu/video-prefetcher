...
export default class VideoPrefetcher extends EventEmitter {
    ...
    start() {
        ...
        var onLoad = (event)=> {
            ...
            if (!(status === 0 || status === 200)) {
                this.emit(VideoPrefetcher.eventTypes.error, new Error("Unexpected status code " + status + " for " + this.originalSrc));
                return;
            }
    ...
    onError(callback) {
        this.on(VideoPrefetcher.eventTypes.error, callback);
    }
